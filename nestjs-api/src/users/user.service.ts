...
import { UserStatus } from 'src/userstatuses/entity/userstatuses.entity';
...
export class UserService {
  ...
  private async getRoleNames(role: string[]): Promise<string[]> {
    const options = await this.roleRepository.find({
      where: { id: In(role.map((id) => parseInt(id))) },
    });
    const optionMap = new Map(
      options.map((option) => [option.id, option.name]),
    );

    return role.map((id) => optionMap.get(parseInt(id)) || 'Unknown');
  }
  ...
  private async resultData(data: User[]) {
    const result = await Promise.all(
      data.map(async (data: any) => {
        ...
        const roleNames = await this.getRoleNames(data.role);
        return {
          id: data.id,
