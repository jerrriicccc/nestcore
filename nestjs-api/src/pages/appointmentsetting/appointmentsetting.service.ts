...
export class AppointmentSettingService {
  ...
  constructor(
    ...
    @InjectRepository(AppointmentSettingEntity)
    private readonly AdditionalServiceRepository: Repository<AppointmentSettingEntity>,
  ) {}
  ...
  async getMainIndexTable(
    entity: EntityTarget<AppointmentSettingEntity>,
    page = 1,
    searchCond = '',
    ...
    try {
      const repo = this.getRepository(entity);
      const queryBuilder = repo.createQueryBuilder('additionalservices');

      const result = await paginate<AppointmentSettingEntity>(queryBuilder, {
